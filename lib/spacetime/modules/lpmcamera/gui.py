...
class CameraFrameGUI(FalseColorImageGUI, CameraGUI, SingleFrameAnimation):
	...
	traits_view = support.PanelView(
		traitsui.Group(
			...
			traitsui.Item('channel', editor=gui.support.RangeEditor(low=0, high_name='channelcount', mode='spinner')),
			traitsui.Item('mode', style='custom'),
			traitsui.Item('firstframe', label='First frame', editor=gui.support.RangeEditor(low=0, high_name='framecount', mode='spinner')),
			traitsui.Item('lastframe', label='Last frame', enabled_when='is_filmstrip', editor=gui.support.RangeEditor(low=0, high_name='framecount', mode='spinner')),
			traitsui.Item('stepframe', label='Key frame mode', enabled_when='is_filmstrip', editor=gui.support.RangeEditor()),
...
class CameraTrendGUI(DoubleTimeTrendGUI, CameraGUI, XlimitsGUI):
	...
	traits_view = support.PanelView(
		traitsui.Group(
			...
			traitsui.Item('filename', editor=support.FileEditor(filter=['Camera RAW files (*.raw)', '*.raw', 'All files', '*'], entries=0)),
			traitsui.Item('firstframe', label='First frame', editor=gui.support.RangeEditor(low=0, high_name='framecount', mode='spinner')),
			traitsui.Item('lastframe', label='Last frame', editor=gui.support.RangeEditor(low=0, high_name='framecount', mode='spinner'), enabled_when='not_fft'),
			traitsui.Item('stepframe', label='Key frame mode', enabled_when='not_fft', editor=gui.support.RangeEditor()),
